 ...
 *
 *  Auf Verwendung der STD-Lib wurde verzichtet, da Irrlicht so gut wie alle Container selbst mitbringt.
 *  Und um die Kompatibilität zu verschiedenen OSs zu gewährleisten. Und weil ich die STD-Lib nicht mag.
 */
 ...
 *
 *  Für die Codeformatierung gelten die Regeln, die direkt im Code abgeschaut werden können.
 *  Für die Architektur gelten weitere Regeln:
 *
 ...
 *  - Nutze ++myVar in Loops, nutze myVar++ wenn myVar der Rückgabewert einer Funktion ist
 *  - Nutze ClassA += ClassB wenn der Constructor von ClassA oder ClassB sehr "teuer" ist
 *  - Versuche, keine lokalen Variablen zu erstellen, wenn diese direkt danach von der Funktion zurückgegeben werden
 *  - Nutze Initialisierungslisten für Klassen, vermeide Member-Assignment, wo es geht
 *  - Für nicht-Integrale Datentypen, versuche Referenzen statt Zeigern zu nutzen
 *  - Für Konstruktoren mit nur einem Parameter, benutze Schlüsselwort "explicit"; außer es ist ein Copy-Constructor
