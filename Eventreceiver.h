 ...
 *  \brief Klasse (Singleton) zur Behandlung von Tastatur/Maus, GUI und anderen Events.
 ...
 *
 *  Events sind Tastatur- und Mauseingaben. GUI-Events (geklickter Button, Menüaufruf usw.) werden gesondert behandelt.
 *  \attention Klasse ist `Singleton`. Klasse ist von IRRLICHT-Klasse abgeleitet und multithreading-fähig.
 *             Sie wird bei Weitem schneller aktualisiert als der Main Loop.
 *  \note Instanzierung: `Eventreceiver& myEventreceiver = Eventreceiver::getInstance();`
...
public:
     ...
     *  \attention Im Main Loop am Ende aller Tastenabfragen aufrufen, um aktuellen Zustand der Tastatur/Maus für
     *             spätere Vergleiche zu speichern!
     *  \param -
     *  \return -
     */
     ...
     *  \attention Das Abschalten von Reaktionen auf Maus-Events wirkt sich nur auf die Maustasten aus,
     *             nicht auf die Mausposition!
     ...
     *  \return -
     */
    void setEventReactionActive( bool keysActive = true, bool mausActive = true, bool guiActive = true );
     ...
     */
    bool hasKeyJustBeenPressedDown( const irr::EKEY_CODE keyCode ) const;

    /*! \brief Abfrage der Änderung des Zustands einer Taste.
     *  \param keyCode (\a const \a EKEY_CODE) TastenCode-Enumerator
     *  \return `true` wenn die Taste gerade noch gedrückt war und jetzt nicht mehr gedrückt ist, ansonsten `false`
     */
    bool hasKeyJustBeenReleased( const irr::EKEY_CODE keyCode ) const;

    /*! \brief Abfrage des aktuellen Zustands der SHIFT-Tasten.
     *  \param -
     *  \return `true` wenn eine oder beide SHIFT-Tasten gedrückt sind, ansonsten `false`
     */
    bool isShiftDown() const;

    /*! \brief Abfrage des vorherigen Zustands der SHIFT-Tasten.
     *  \param -
     *  \return `true` wenn eine oder beide SHIFT-Tasten gedrückt waren, ansonsten `false`
     */
    bool wasShiftDown() const;

    /*! \brief Abfrage des aktuellen Zustands der STRG-Tasten.
     *  \param -
     *  \return `true` wenn eine oder beide STRG-Tasten gedrückt sind, ansonsten `false`
     */
    bool isCtrlDown() const;

    /*! \brief Abfrage des vorherigen Zustands der STRG-Tasten.
     *  \param -
     *  \return `true` wenn eine oder beide STRG-Tasten gedrückt waren, ansonsten `false`
     */
     ...
     *  \note Besser virtual core::position2d<s32> device->getCursorControl()->getPosition() verwenden!
     *  \param -
     *  \return \a s32 aktuelle X-Koordinate
     */
    irr::s32 getMouseX() const;
     ...
     */
    irr::s32 getMouseY() const;

    /*! \brief Gibt die vorherige X-Koordinate des Mauspfeils zurück.
     *  \param -
     *  \return \a s32 vorherige X-Koordinate
     */
    irr::s32 getMouseLastX() const;

    /*! \brief Gibt die vorherige Y-Koordinate des Mauspfeils zurück.
     *  \param -
     *  \return \a s32 vorherige Y-Koordinate
     */
    irr::s32 getMouseLastY() const;

    /*! \brief Gibt das aktuelle Delta des Mausrades zurück.
     *  \param -
     *  \return \a f32 um wieviel wurde das Mausrad bewegt
     */
    irr::f32 getMouseWheel() const;

private:

    bool KeyIsDown_[ irr::KEY_KEY_CODES_COUNT ];  // TastenCode-Liste mit den aktuellen Werten
    bool KeyLastState_[ irr::KEY_KEY_CODES_COUNT] ;  // TastenCode-Liste mit den vorhergehenden Werten.
    // "KeyLastState" ist eine Bremse, sie dient zur synchronisierung.
    // Absolut notwendig, um Statuswechsel von Tasten in "Main-Loop-Speed" abfragen zu können.
    // Denn: der EventReceiver im Hintergrund ist immer schneller als unser Main Loop.
    // Darum würden wir Änderungen (Tastendrücke) gar nicht mitbekommen.
    ...
    irr::s32 mouseX_;  // aktuelle X-Koordinate des Mauspfeils
    irr::s32 mouseLastX_;  // vorherige X-Koordinate des Mauspfeils
    irr::s32 mouseY_;  // aktuelle Y-Koordinate des Mauspfeils
    irr::s32 mouseLastY_;  // vorherige Y-Koordinate des Mauspfeils
    irr::f32 mouseWheel_;  // in welche Richtung wurde das Scrollrad um wieviel bewegt
    ...
    Eventreceiver& operator=( const Eventreceiver& );  // Instanz ist nicht zuweisbar
